    ...
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            for (int i = 1; i < properties.Length; i++)
            {
                ...
                {
                    values += ($"\"{((DateTime)property.GetValue(obj)).Year}-{((DateTime)property.GetValue(obj)).Month}-{((DateTime)property.GetValue(obj)).Day}\"");
                ...
                {
                    values += ($"\"{property.GetValue(obj).ToString()}\"");
                    if (i != properties.Length - 1)
                    {
                        values += ",";
                    }
